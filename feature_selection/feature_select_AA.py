...
for train_index, test_index in skf.split(X, target):
    ...
    np.savetxt("/project/gpaa/machine_learning/jainam_capstone/feature_selection/feature_selection_results/top_240_indices_AA_LASSO_split_"+str(split)+".csv", ranks2.argsort()[-240:][::-1], delimiter=",")
    split += 1
########################### Random Forest ####################################
split = 1    
for train_index, test_index in skf.split(X, target):
    ...
    RF_to_write = ranks3
    np.savetxt("/project/gpaa/machine_learning/jainam_capstone/feature_selection/feature_selection_results/top_240_indices_AA_random_forest_split_"+str(split)+".csv", ranks3.argsort()[-240:][::-1], delimiter=",")
    split += 1
...
for train_index, test_index in skf.split(X, target):
    ...
    rfe_to_write = ranks1
    np.savetxt("/project/gpaa/machine_learning/jainam_capstone/feature_selection/feature_selection_results/top_240_indices_AA_rfe_split_"+str(split)+".csv", ranks1.argsort()[-240:][::-1], delimiter=",")
    split += 1

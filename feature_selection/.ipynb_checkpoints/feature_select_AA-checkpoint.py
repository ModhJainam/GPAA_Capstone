...
with open('/gpfs/gpfs0/project/gpaa/machine_learning/jainam_capstone/feature_selection/train_test_splits_AA_b16_new.csv', 'r') as f:
...
df_binary_labeled = df_labeled.drop(columns=['sample_id', 'AGE', 'white', 'black', 'hispanic', 'male', 'female'], axis=1)
...
for train_index, test_index in skf.split(X, target):
    ...
    np.savetxt("/project/gpaa/machine_learning/jainam_capstone/feature_selection/feature_selection_results/top_240_indices_AA_RLASSO_split_"+str(split)+".csv", ranks2.argsort()[-240:][::-1], delimiter=",")
    split += 1
'''
############################ LASSO ##########################################
for split in range(1,6):
    ...
    random.seed(42)
    lasso = Lasso(random_state=101, selection = 'random')
    search = GridSearchCV(estimator=lasso,param_grid= {'alpha': np.arange(0.1,10,0.1)}, cv = 5, scoring="neg_mean_squared_error", n_jobs = -1)
    search.fit(X_train, y_train)
    print(search.best_params_)
    
    lasso_rank = np.abs(search.best_estimator_.coef_).tolist()
    
    ranks2 = rankdata(lasso_rank)
    LASSO_to_write = ranks2
    
    np.savetxt("/project/gpaa/machine_learning/jainam_capstone/feature_selection/feature_selection_results_new/ranked_indices_AA_LASSO_split_"+str(split)+".csv", ranks2.argsort()[:][::-1], delimiter=",")
'''
############################ Relaxed LARS LASSO (ReLASSO)##########################################
for split in range(1,6):
    train_col = [col for col in train_test_splits.columns if str(split)+'_train' in col]
    train_indices = list(train_test_splits[train_col[0]].dropna())
    test_col = [col for col in train_test_splits.columns if str(split)+'_test' in col]
    ...
    random.seed(42)
    relasso = RelaxedLassoLarsCV(cv = 5, n_jobs = -1)
    relasso.fit(X_train, y_train)
    
    relasso_rank = np.abs(relasso.coef_).tolist()
    ranks2 = rankdata(relasso_rank)
    ReLASSO_to_write = ranks2
    
    np.savetxt("/project/gpaa/machine_learning/jainam_capstone/feature_selection/feature_selection_results_new/top_240_indices_AA_ReLASSO_split_"+str(split)+".csv", ranks2.argsort()[-240:][::-1], delimiter=",")
'''    
...
for split in range(1,6):
    ...
    RF_to_write = ranks3
    np.savetxt("/project/gpaa/machine_learning/jainam_capstone/feature_selection/feature_selection_results_new/ranked_indices_AA_random_forest_split_"+str(split)+".csv", ranks3.argsort()[:][::-1], delimiter=",")
    split += 1
'''    
############################## NSF ###########################################
...
for split in range(1,6):
    ...
    rfe_to_write = ranks1
    np.savetxt("/project/gpaa/machine_learning/jainam_capstone/feature_selection/feature_selection_results_new/ranked_indices_AA_rfe_split_"+str(split)+".csv", ranks1.argsort()[:][::-1], delimiter=",")
    split += 1  
